...
d= {'a': 0.003, 'b_a': 0.002, 'd': 0.01, 'm_f': 0.01, 's_m': 0.05, 's_p': 0.3, 'f_s': 0.1, 'c_s': 0.0812, 'b_v': 0.6268382560000001, 'v_p': 0.11382846093000482, 'h': 0.6079227848, 's_s': 0.24653028099513596, 's_t': 0.00278, 's_q': 0.22789214360201268, 'c_c': 0.13812964122472848, 'car_s': 0.030807847809066112}

d_1 = {k: round(v, 7) for k,v in d.items()}
r_1 = {k: round(v, 7) for k,v in result.items()}


print(dict(sorted(d_1.items(), key=lambda item: item[1])))
print(dict(sorted(r_1.items(), key=lambda item: item[1])))
